...
@Slf4j
class SubjectRepositoryIT extends BaseRepositoryIT {
	...
	@DisplayName("Successfully save a SUBJECT node")
	@Test
	...
	@DisplayName("Successfully save a SUBJECT node with a related DEPARTMENT node")
	@Test
	void validSaveTestWithDepartment() throws Exception {
		Subject entity = TestDataFactoryUtil.generateSubjectInstanceWithDepartment();
		repo.save(entity).subscribe(
				value -> {
					assertFalse(StringUtils.isBlank(value.getId()));
					assertFalse(StringUtils.isBlank(value.getDepartment().getId()));
				},
				error -> log.error("{}", error),
				() -> log.warn("Completed without value nor error"));
	}
