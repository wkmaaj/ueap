...
@Slf4j
@Service
class VehicleRelationshipsValidatorService implements ValidateEntityRelationshipsService<Vehicle> {
	...
	@Autowired
	VehicleRelationshipsValidatorService(
	...
	@Override
	public Vehicle validateEntityRelationships(Vehicle vehicle) {
		if(vehicle.getPerson() != null) {
			if(log.isTraceEnabled()) {
				log.trace("Updating entity: {}", vehicle);
			}
			vehicle.getPerson().setNodeUid(personNodeUidGeneratorService.generateNodeUid(vehicle.getPerson()));
			vehicle.setPerson(personNodeUidValidatorService.validateNodeUid(vehicle.getPerson()));
			if(log.isDebugEnabled()) {
				log.debug("Updated entity: {}", vehicle);
			}
		}
